...
namespace DataStructures
{
	...
	public:
		...
		node* Del( const BinarySearchTreeType& input, bool *deleted = 0 );
		...
		private:
			...
			struct StackNode
			{
				...
				StackNode(node *_node, NodeDirection _direction)
				{
					treeNode = _node;
		...
		public:
			SimpleIterator(BinarySearchTree *tree)
			{
				binarySearchTree = tree;
				if (tree->BinarySearchTree_size)
					nodeStack.Push(StackNode(tree->root, DIR_SELF));
				else
					nodeStack.Clear();
			}
			...
			{
				nodeStack.Clear();
				SimpleIterator::SimpleIterator(binarySearchTree);
			}
			BinarySearchTreeType *GetData()
			{
				if (nodeStack.IsEmpty())
					return 0;
				else
					return nodeStack.Peek().treeNode->item;
			}
			void FindNext()
			{
				while (!nodeStack.IsEmpty())
				{
					//pop current node at the very first
					StackNode stackNode = nodeStack.Pop();
					switch (stackNode.direction)
					{
					case DIR_SELF:
						//direction == DIR_SELF means the sub-nodes never been touched 
						if (stackNode.treeNode->left)
						{
							//change node's direction and push it back
							stackNode.direction = DIR_LEFT;
							nodeStack.Push(stackNode);
							//push the left node, then
							nodeStack.Push(StackNode(stackNode.treeNode->left, DIR_SELF));
							return;
						}
						else
						{
							if (stackNode.treeNode->right)
							{
								//change node's direction and push it back
								stackNode.direction = DIR_RIGHT;
								nodeStack.Push(stackNode);
								//push the left node, then
								nodeStack.Push(StackNode(stackNode.treeNode->right, DIR_SELF));
								return;
							}
							//no sub-node(s) found, loop continues, try to pop up a new node
						}
						break;
					case DIR_LEFT:
						if (stackNode.treeNode->right)
	...
	template <class BinarySearchTreeType>
	bool AVLBalancedBinarySearchTree<BinarySearchTreeType>::Add ( const BinarySearchTreeType& input )
	{
	...
	template <class BinarySearchTreeType>
	bool AVLBalancedBinarySearchTree<BinarySearchTreeType>::Del( const BinarySearchTreeType& input )
	{
		bool deleted;
		typename BinarySearchTree<BinarySearchTreeType>::node * current = BinarySearchTree<BinarySearchTreeType>::Del( input, &deleted );
	...
	{
		typename BinarySearchTree::node * tempnode;
		BinarySearchTreeType* dummyptr = 0;
