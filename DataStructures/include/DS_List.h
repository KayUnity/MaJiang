...
/// Bug : OrderedList<HexString> inserting duplicate data will be failed. need Initialize when realloced
///       See Insert( const list_type &input, const unsigned int position )
...
static const __u32 MAX_UNSIGNED_LONG = 4294967295U;
static const __u64 MAX_UNSIGNED_LONGLONG = 0xffffffffffffffff;
...
namespace DataStructures
{
    ...
    List<list_type>::List( const List& original_copy )
    {
        ...
        {
            listArray = (list_type *)malloc(sizeof(list_type) * original_copy.list_size);
            for (unsigned int i=0; i<original_copy.list_size; i++)
                new (&listArray[i]) list_type;
    ...
    void List<list_type>::Insert( const list_type &input, const unsigned int position )
    {
        ...
        if ( list_size == allocation_size )
        {
            ...
            listArray = (list_type *)realloc(listArray, sizeof(list_type) * allocation_size);
			for (unsigned int i = list_size; i<allocation_size; i++)	// added by Kay 2016/05/13
				new (&listArray[i]) list_type;							// initialize, for HexString
        }
    ...
    void List<list_type>::Insert( const list_type &input )
    {
        ...
        if ( list_size == allocation_size )
        {
            ...
            listArray = (list_type *)realloc(listArray, sizeof(list_type) * allocation_size);
            for (unsigned int i=list_size; i<allocation_size; i++)
                new (&listArray[i]) list_type;
        }
    ...
    void List<list_type>::Preallocate( unsigned countNeeded )
    {
        ...
        {
            // allocate twice the currently allocated memory
            unsigned int old_size = allocation_size;
            allocation_size=amountToAllocate;
            listArray = (list_type *)realloc(listArray, sizeof(list_type) * allocation_size);
            for (unsigned int i=old_size; i<allocation_size; i++)
                new (&listArray[i]) list_type;
        }
    }
